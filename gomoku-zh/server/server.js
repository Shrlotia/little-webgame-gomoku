  ...
  ws.on("message", (raw) => {
    ...
    if (type === "createRoom") {
      ...
      const room = {
        ...
        clients: new Set([ws]),
        undoCounts: { 1: 0, 2: 0 }, // 每位玩家本局已用悔棋次數
    ...
    if (type === "requestProposal") {
      ...
      if (!room.players[toSeat]) {
        ws.send(JSON.stringify({ type: "error", message: "對方尚未在房間" }));
        return;
      }
      ...
      if (action === "undo") {
        if (room.winner) {
          ws.send(JSON.stringify({ type: "error", message: "勝負已定，不可悔棋" }));
          return;
        }
        if (room.history.length === 0) {
          ws.send(JSON.stringify({ type: "error", message: "沒有可悔的一步" }));
          return;
        }
        if ((room.undoCounts[fromSeat] || 0) >= MAX_UNDO) {
          ws.send(JSON.stringify({ type: "error", message: "已達悔棋次數上限" }));
          return;
        }
      }
